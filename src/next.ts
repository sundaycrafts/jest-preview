...
export async function configureNextJestPreview(
  ...
  if (config.transform) {
    config.transform['^.+\\.(css|scss|sass)$'] = '@sundaycrafts/jest-preview/transforms/css';
    config.transform['^(?!.*\\.(js|jsx|mjs|cjs|ts|tsx|css|json)$)'] = '@sundaycrafts/jest-preview/transforms/file';
  }
